...
uses
  ...
  Menus, ShellApi, ExtCtrls, Dialogs,SimpleTimer {$IFDEF DELPHI_4_UP}, ImgList{$ENDIF};
...
{$ENDIF UNICODE}
  ...
  THintString = widestring;       // 128 bytes, last char should be #0
...
{$ENDIF}
  ...
  protected
    IconData: TNotifyIconDataw;       // Data of the tray icon wnd.
...
{$WARNINGS ON}
  ...
  IconData.cbSize := SizeOf(TNotifyIconDataw);
...
//        Exit;
    ...
    begin
      for x:=0 to length(Fhint)-1 do //F0X CHANGED
        IconData.szTip[x]:=Fhint[x+1];
      ...
      for x:=0 to length(icondata.szTip)-1 do //F0X CHANGED
        IconData.szTip[x]:=#0;
...
begin
  ...
  begin
    ...
    begin
      if SettingPreview then
        if InitIcon then
          Result := Shell_NotifyIconw(NIM_DELETE, @IconData);
    end
    else
    if InitIcon then
      Result := Shell_NotifyIconw(NIM_DELETE, @IconData);
...
begin
  Result := False;
  if InitIcon then
    Result := Shell_NotifyIconw(NIM_MODIFY, @IconData);
end;
...
begin
  ...
  begin
    ...
    for I := 0 to Length(szInfo)-1 do //TEXT
      szInfo[I] := wideChar(Text[I]);
    szInfo[0] := #1;
    FillChar(szInfoTitle, 0, SizeOf(szInfoTitle));
    for I := 0 to Length(szInfoTitle)-1 do //TITLE
      szInfoTitle[I] := wideChar(Title[I]);
    szInfoTitle[0] := #1;
    TimeoutOrVersion.uTimeout := TimeoutSecs * 1000;
    dwInfoFlags := aBalloonIconTypes[IconType];
  end;
...
begin
  Result := Shell_NotifyIconw(NIM_SETFOCUS, @IconData);
end;
