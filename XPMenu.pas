...
uses
  ...
  Menus, Commctrl, ExtCtrls, StdCtrls, Buttons, Tntgraphics, tntdialogs, dialogs;
...
//   FTransparentColor: TColor;// +jt
  ...
  protected
    ...
    procedure DrawTheText(Sender: TObject; txt, ShortCuttext: ansistring;
...
procedure TXPMenu.MeasureItem(Sender: TObject; ACanvas: TCanvas;
  var Width, Height: Integer);
var
  s: widestring;//F0X FIX
  a: ansistring;//F0X FIX
  W, H: integer;
  P: TPoint;
  IsLine: boolean;
  FTopMenu: boolean; // +jt
...
begin
  ...
  if FActive then
  begin
    a:=TMenuItem(Sender).Caption;
    {if TMenuItem(Sender).Caption[len]='?' then begin
      //a:=trim(a);
      wideshowmessage(a);
      for w:=1 to len do
        wideshowmessage(a[w]);
      ...
      //a:=copy(a,1,len-2);
      //wideshowmessage('convert '+a+' > '+UTF8Decode(a));
    end;  }

    //showmessage(TMenuItem(Sender).Caption);
    //showmessage(a);
    S:=UTF8Decode(a); //F0X FIX
    //wideshowmessage(s);
    //wideshowmessage(s); //WRONG CONVERSION
    ...
    W := WideCanvasTextWidth(Acanvas,s);//F0X FIX
    //W := ACanvas.TextWidth(s);
    Inc(W, 5);
    if pos('&', s) > 0 then
      W := W - WideCanvasTextWidth(Acanvas,'&');
...
// +jt
    ...
    begin
      H:=WideCanvasTextHeight(ACanvas,s) + Round(Widecanvastextheight(Acanvas,s)*0.75);
...
procedure TXPMenu.DrawTheText(Sender: TObject; txt, ShortCuttext: ansistring;
    ACanvas: TCanvas; TextRect: TRect;
    Selected, Enabled, Default, TopMenu, IsRightToLeft: boolean;
    var TxtFont: TFont; TextFormat: integer);
var
...
begin
  ...
  //      B.Canvas.Font := FFont; //felix added for resolving font problems in Win98
        ...
        DrawtextExw(B.Canvas.Handle,  //F0X FIX
          PwideChar(utf8decode(txt)),
          Length(utf8Decode(txt)),
